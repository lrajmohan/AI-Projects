...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:   #dist to ghost
      allPossible[self.post] = 1.0
    else:
      for p in self.legalPositions:  #p-ghostposition
        trueDistance = util.manhattanDistance(p, pacmanPosition)
        allPossible[p] = self.beliefs[p] * emissionModel[trueDistance] #update belief
  ...
  def elapseTime(self, gameState):
    ...
    for oldghostpost in self.legalPositions:

      Ghostposition = self.setGhostPosition(gameState, oldghostpost)
      newposdistr = self.getPositionDistribution(Ghostposition)
      b = self.beliefs[oldghostpost]   #oldbelief
      for newpos, prob in newposdistr.items():
        allPossible[newpos] += b * prob #newbelief
